---
title: "Avaliação por Fluxos de Caixa Descontados"
subtitle: "@ross, capítulo 06"
author: "Marcelo S. Perlin"
date: last-modified
date-format: DD/MM/YYYY
institute: EA-UFRGS
format:
  revealjs: 
    include-after-body: "content/clock.txt"
    theme: "templates/theme-msperlin.scss"
footer: 'Administração Financeira - UFRGS'
self-contained: true
execute: 
  echo: false
bibliography: "templates/bibfile.bib"
logo: "templates/ufrgs3.jpg"
toc: true
toc-title: "Sumário da aula"
toc-expand: 2
toc-depth: 1
slide-number: true
---

## Objetivos {.unlisted}

```{r}
classtools::setup_quarto_slides("templates")

l_selic <- classtools::get_selic_rate()
l_poup <- classtools::get_poupanca_rate()
```

- Como determinar os valores presente e futuro de investimentos com vários fluxos de caixa.
- Como calcular os pagamentos e as taxas de juros de empréstimos.
- Como empréstimos são amortizados ou completamente pagos.
- Como as taxas de juros são cotadas (da maneira certa e errada).


# Valor presente de múltiplos períodos

## Fórmula VP

$$VP = \sum_{t=1}^{T} \frac{FC_t}{(1+r)^t}$$

$VP$ - Valor presente dos múltiplos fluxos de caixa

$FC_t$ - fluxo de caixa no tempo $t$

$r$ - taxa de juros

$T$ - número de períodos

## Exemplo

```{r}

my_T <- 5
fc <- 5000
r <- l_selic$aa

df_fc <- tibble::tibble(
  Tempo = 0:my_T,
  Valor = c(0, seq(1000, 1500, length.out = my_T))
)

```

> Imagine que ao longo dos próximos `r my_T` anos você irá receber dinheiro a cada ano, conforme tabela abaixo. Assumindo uma taxa de juros de `r classtools::format_percent(r)`, qual o valor presente desses fluxos de caixa?

```{r}
df_fc |>
  gt::gt() |>
  gt::tab_header("Fluxos de caixa") |>
  gt::fmt_currency(columns = dplyr::where(is.numeric) & !dplyr::matches("^Tempo$"), currency = "BRL", locale = "pt")
```

## Solução

```{r}
df_fc <- df_fc |>
  dplyr::mutate(`Valor descontado` = Valor/(1+r)^Tempo,
                `Valor acumulado` = cumsum(`Valor descontado`))

vp <- sum(df_fc$`Valor descontado`)

df_fc |>
  gt::gt() |>
  gt::tab_header("Fluxos de caixa",
                 glue::glue('custo de capital = {classtools::format_percent(r)}')) |>
  gt::fmt_currency(columns = dplyr::where(is.numeric) & !dplyr::matches("^Tempo$"), currency = "BRL", locale = "pt")
```

O valor presente dos fluxos de caixa é `r  classtools::format_cash(vp)`.

## Anuidades

```{r}
cf <- 1000
my_T <- 10

l_out <- classtools::create_cashflow_plot(0, rep(cf, my_T-1), cf)
```


> Uma anuidade é um fluxo de caixa constante e igual para todo o período

Um exemplo é receber `r classtools::format_cash(cf)` por ano, ao longo de `r my_T` anos:

```{r}
l_out$p
```

## Solução

Usando a fórmula anterior e assumindo $FC_t = FC$:

$$VP = \sum_{t=1}^{T} \frac{FC}{(1+r)^t}$$

Simplificando a fórmula: 

$$VP = FC \frac{1 - \frac{1}{(1+r)^T}}{r}$$

## Exemplo anuidade

> Imagine que ao longo dos próximos `r my_T` anos você irá receber dinheiro a cada ano, conforme tabela abaixo. Assumindo uma taxa de juros de `r classtools::format_percent(r)`, qual o valor presente desses fluxos de caixa?


```{r}
my_T <- 5
fc <- 5000
r <- l_selic$aa

df_fc <- tibble::tibble(
  Tempo = 0:my_T,
  Valor = c(0, rep(fc, my_T))
)


df_fc <- df_fc |>
  dplyr::mutate(`Valor descontado` = Valor/(1+r)^Tempo,
                `Valor acumulado` = cumsum(`Valor descontado`))

vp <- sum(df_fc$`Valor descontado`)

df_fc |>
  gt::gt() |>
  gt::tab_header("Fluxos de caixa",
                 glue::glue('custo de capital = {classtools::format_percent(r)}')) |>
  gt::fmt_currency(columns = dplyr::where(is.numeric) & !dplyr::matches("^Tempo$"), currency = "BRL", locale = "pt")
```

O valor presente dos fluxos de caixa é `r  classtools::format_cash(vp)`.

## Perpetuidades

> Perpetuidades são fluxos de caixa constantes e perpétuos (nunca terminam)

Substituindo $T = \infty$ na fórmula do VP, temos:

$$VP = \sum_{t=1}^{\infty} \frac{FC}{(1+r)^t}$$

Simplificando e levando a equação anterior ao limite onde $T$ aproxima-se do infinito ($\infty$), temos:

$$VP=\frac{FC}{r}$$


# Valor futuro de múltiplos períodos

## Fórmula $VF$

$$VF_T = \sum_{t=1}^{T} FC_t (1+r)^{T-t}$$

$VF_T$ - Valor futuro total acumulado no tempo $T$

$FC_t$ - fluxo de caixa no tempo $t$

$r$ - taxa de juros

$T$ - número de períodos

## Ilustração

```{r}
#| fig-cap: !expr classtools::cite_ross(150)

knitr::include_graphics("figs/vf-multiplos-periodos-02.png")
```


## Exemplo cálculo

```{r}
my_T <- 10
fc <- 1000
r <- l_poup$am

df_fc <- tibble::tibble(
  Tempo = 0:my_T,
  Valor = c(0, rep(fc, my_T))
)

```

> Imagine que você irá depositar `r classtools::format_cash(fc)` por mês na poupança pelos próximos `r my_T` meses. Ao final do `r my_T`º mês, qual o valor encontrado na conta poupança?

. . .

```{r}
df_fc |>
  gt::gt() |>
  gt::tab_header("Fluxos de caixa") |>
  gt::fmt_currency(columns = dplyr::where(is.numeric) & !dplyr::matches("^Tempo$"), currency = "BRL", locale = "pt")
```

## Solução

```{r}
df_fc <- df_fc |>
  dplyr::mutate(`VF` = Valor*(1+r)^(my_T - Tempo),
                `VF acumulado` = cumsum(VF))

vf <- sum(df_fc$`VF`)

df_fc |>
  gt::gt() |>
  gt::tab_header("Fluxos de caixa",
                 glue::glue('Taxa de juros = {classtools::format_percent(r)}')) |>
  gt::fmt_currency(columns = dplyr::where(is.numeric) & !dplyr::matches("^Tempo$"), currency = "BRL", locale = "pt")
```

O valor futuro dos fluxos de caixa é `r  classtools::format_cash(vf)`.



##  A importância do reinvestimento dos dividendos

> Investimentos em renda variável (Ações e FIIs) dão direito ao recebimento de dividendos (parcelas do lucro)

::: {.incremental}
- Esses dividendos variam de valor de acordo com o lucro e a decisão de payout

- A frequência de pagamento também varia (empresas decidem quando e como vão pagar)

- Esse dinheiro pode ser gasto ou reinvestido

- O não reinvestimento causa grande impacto no patrimônio final
:::

## O caso da EGIE3

```{r}
ticker <- "EGIE3"
first_date <- "2015-01-01"

l_out <- classtools::make_cashflow_plot(ticker, 
                                        'SA',
                                        first_date)

CF <- l_out$CF
total_divs <- sum(CF$CF[2:(nrow(CF)-1)])
P_0 <- -CF$CF[1]
P_T <- CF$CF[nrow(CF)]

ret_investidor <- (total_divs + P_T)/P_0 - 1

l_out$p
```

##  O impacto do não reinvestimento de dividendos

```{r}
library(dplyr)
library(ggplot2)

ticker <- "EGIE3.SA"
first_date <- "2015-01-01"

l_out <- classtools::make_cashflow_plot(ticker, 'SA', first_date)

df_yf <- l_out$df_prices |>
  dplyr::mutate(cumret_close = price_close/first(price_close),
                cumret_adj = adjusted_close/first(adjusted_close))

df_plot <- tibble::tibble(
  prices = c(df_yf$cumret_close, df_yf$cumret_adj),
  ref_date = rep(df_yf$ref_date, 2),
  type = c(rep("Sem reinvestimento de dividendo", nrow(df_yf)),
           rep("Reinvestindo dividendo", nrow(df_yf))
           )
)

p <- ggplot(df_plot, aes(x = ref_date, y = prices, color = type)) + 
  geom_line() + 
  labs(title = paste0("Efeito do reinvestimento do dividendo (", ticker, ")"),
       x = 'Dias',
       y = "Investimento acumulado",
       color = "Tipo") + 
  scale_y_continuous(labels = classtools::format_percent) + 
  theme_light()

p
```


# Mudança de taxa de retorno

## Mensal para anual

```{r}
selic_aa <- dplyr::last(l_selic$aa)
selic_am <- l_selic$am
```

Dada uma taxa SELIC hoje de `r classtools::format_percent(selic_aa)` ao ano:

:::: {.columns}

::: {.column width="50%"}
**Anual para mensal**

$$r_{am} = (1+r_{aa})^{1/12} - 1$$

Um retorno anual de `r classtools::format_percent(selic_aa)` equivale a `r classtools::format_percent(selic_am)` ao mês. 
:::

::: {.column width="50%"}
**Mensal para anual**

$$r_{aa} = (1+r_{am})^{12} - 1$$

```{r}
r <- 0.2
```


Um retorno mensal de `r classtools::format_percent(selic_am)` equivale a `r classtools::format_percent(selic_aa)` anualmente.

:::

::::


# Custo de uma dívida

## CET

> O CET, **Custo Efetivo Total** é a taxa de juros **real** do empréstimo, incluindo **todas as tarifas e comissões cobradas**.

- O CET é utilizado em:
  - financiamento imobiliário
  - financiamentos em geral
  - cartão de crédito

. . .

::: {.callout-caution}
## Cuidado!
Apesar de ser legalmente obrigado a divulgar a taxa CET em qualquer empréstimo, o banco/vendedor geralmente esconde a CET até a assinatura do contrato!
:::

## Exemplo CET

```{r}
debt <- 10000
r_nominal <- l_selic$aa
my_T <- 3

vf <- debt*(1+r_nominal)^my_T
cost_register <- 150
cost_credit <- 500

true_debt <- debt - cost_register - cost_credit
```


> Imagine pegar `r classtools::format_cash(debt)` emprestado no banco a uma taxa nominal de `r classtools::format_percent(r_nominal)` ao ano (SELIC), com pagamento em `r my_T` anos.

. . .

O valor futuro (VF) a ser pago no ano `r my_T` é de `r classtools::format_cash(vf)`. Porém, o banco tem os seguintes custos na operação de concessão de empréstimo:

1) taxa fixa de registro de `r classtools::format_cash(cost_register)`
2) custo fixo de análise de crédito de `r classtools::format_cash(cost_credit)`

. . .

> No início da operação, o valor obtido do financiamento será de `r classtools::format_cash(true_debt)`, e não `r classtools::format_cash(debt)`.

## Solução CET

Os fluxos de caixa são:

```{r}
received <- debt - cost_credit - cost_register
l_p <- classtools::create_cashflow_plot(-received, rep(0, my_T-1),
                                        -vf)

l_p$p

cet <- (vf/received)^(1/my_T) - 1
```

Enquanto a taxa de juros nominal era de  `r classtools::format_percent(r_nominal)`, a CET é equivalente a `r classtools::format_percent(cet)`.

## CET Real em Financiamento Imobiliário

```{r}
principal_cet <- 200000
n_months_cet <- 360
r_annual_cet <- 0.09
r_monthly_cet <- (1 + r_annual_cet)^(1/12) - 1

pmt_base_cet <- principal_cet * (r_monthly_cet * (1 + r_monthly_cet)^n_months_cet) / ((1 + r_monthly_cet)^n_months_cet - 1)

# Custos adicionais obrigatórios
mip <- 60    # Seguro Morte e Invalidez Permanente
dfi <- 25    # Seguro Danos Físicos ao Imóvel
admin <- 25  # Taxa mensal de administração bancária

pmt_total_cet <- pmt_base_cet + mip + dfi + admin

cfs_cet <- c(principal_cet, rep(-pmt_total_cet, n_months_cet))
cet_m_real <- FinCal::irr(cfs_cet)
cet_aa_real <- (1 + cet_m_real)^12 - 1
```

> Em um financiamento de **`r classtools::format_cash(principal_cet)`** a uma taxa nominal de **`r classtools::format_percent(r_annual_cet)` a.a.** (parcela base: `r classtools::format_cash(pmt_base_cet)`):

- **Custos adicionais mensais**:
  - Seguro MIP (Morte e Invalidez): `r classtools::format_cash(mip)`/mês
  - Seguro DFI (Danos Físicos ao Imóvel): `r classtools::format_cash(dfi)`/mês
  - Taxa de Administração Bancária: `r classtools::format_cash(admin)`/mês
- **Prestação total real**: **`r classtools::format_cash(pmt_total_cet)`**/mês
- **CET Efetivo Real**: **`r classtools::format_percent(cet_aa_real)` a.a.** (em vez de `r classtools::format_percent(r_annual_cet)` a.a.).


# Tipos de empréstimos

## Empréstimos tipo desconto

> O empréstimo tipo desconto é a forma mais simples de empréstimo. O mutuário recebe o dinheiro hoje e o paga em uma única parcela no futuro. 

```{r}
cf_0 <- 750
cf_t <- 0
cf_T <- 1000
my_T <- 5

l_p <- classtools::create_cashflow_plot(cf_0, rep(cf_t, my_T), cf_T)

l_p$p
```

## Cálculo da CET

> Dado um empréstimo de recebimento de `r classtools::format_cash(cf_0)` hoje, com pagamento de `r classtools::format_cash(cf_T)` em `r my_T` anos, qual o custo efetivo da dívida?

. . .

```{r}
cet <- FinCal::irr(l_p$cashflows$CF)
```


Calculando a TIR, temos que:

CET = `r classtools::format_percent(cet)`

## Empréstimos com juros constantes

> Empréstimos com juros constantes são aqueles em que o mutuário paga somente juros a cada período e paga todo o principal (o montante original do empréstimo) em algum momento futuro.

```{r}
cf_0 <- -750
cf_t <- -50
cf_T <- -1000
my_T <- 5
custo <- 0

l_p <- classtools::create_cashflow_plot(cf_0-custo, rep(cf_t, my_T), cf_T)

l_p$p

cet <- FinCal::irr(l_p$cashflows$CF)
```

Aplicando a fórmula da TIR, temos que o CET é igual a `r classtools::format_percent(cet)`.


## Empréstimos com pagamento parcelado

> É aquele onde o credor exige do mutuário o pagamento de partes do montante do empréstimo ao longo do período do empréstimo. 

```{r}
cf_0 <- -750
cf_t <- -250
cf_T <- cf_t
my_T <- 5
custo <- 0

l_p <- classtools::create_cashflow_plot(cf_0-custo, rep(cf_t, my_T), cf_T)

l_p$p

cet <- FinCal::irr(l_p$cashflows$CF)
```
Aplicando a fórmula da TIR, temos que o CET é igual a `r classtools::format_percent(cet)`.

# Financiamento Imobiliário

## SAC X PRICE

:::: {.columns}

::: {.column width="50%"}
- **Sistema de Amortização Constante (SAC)**
  - o tomador paga prestações decrescentes. 
  - a dívida é amortizada de forma constante
:::

::: {.column width="50%"}
- **Tabela PRICE**
  - mantém as parcelas constantes
  - geralmente vale para prazos maiores
:::

::::

## Exemplo Comparativo: SAC x PRICE

```{r}
principal <- 300000
n_months <- 30*12
r_annual <- 0.12
r_monthly <- (1 + r_annual)^(1/12) - 1

# PRICE
pmt_price <- principal * (r_monthly * (1 + r_monthly)^n_months) / ((1 + r_monthly)^n_months - 1)
total_price <- pmt_price * n_months
juros_price <- total_price - principal

# SAC
amort_sac <- principal / n_months
pmt_sac_1 <- amort_sac + principal * r_monthly
pmt_sac_n <- amort_sac + (principal - (n_months - 1) * amort_sac) * r_monthly
total_sac <- sum(sapply(1:n_months, function(m) amort_sac + (principal - (m - 1) * amort_sac) * r_monthly))
juros_sac <- total_sac - principal
```

> Imagine um financiamento de **`r classtools::format_cash(principal)`**, CET de **`r classtools::format_percent(r_annual)` a.a.** e prazo de **`r n_months` meses**:

- **Tabela PRICE**:
  - Parcela constante de **`r classtools::format_cash(pmt_price)`** ao mês.
  - Total pago ao final: **`r classtools::format_cash(total_price)`** (sendo `r classtools::format_cash(juros_price)` de juros).

- **Sistema SAC**:
  - Amortização constante de **`r classtools::format_cash(amort_sac)`** ao mês.
  - 1ª parcela de **`r classtools::format_cash(pmt_sac_1)`** e última parcela de **`r classtools::format_cash(pmt_sac_n)`**.
  - Total pago ao final: **`r classtools::format_cash(total_sac)`** (sendo `r classtools::format_cash(juros_sac)` de juros).

::: {.callout-note}
**Nota sobre a taxa de juros**: Considerou-se a taxa efetiva mensal $r_{am} = (1 + 8\%)^{1/12} - 1 \approx 0,6434\%$ a.m.
:::


## Gráfico com parcelas (primeiros 24 meses)

```{r}
library(ggplot2)
library(dplyr)

n_plot <- 24

# PRICE
df_price <- tibble::tibble(
  mes = 1:n_plot,
  parcela = rep(pmt_price, n_plot),
  sistema = "PRICE"
)

# SAC
pmt_sac_vec <- sapply(1:n_plot, function(m) amort_sac + (principal - (m - 1) * amort_sac) * r_monthly)
df_sac <- tibble::tibble(
  mes = 1:n_plot,
  parcela = pmt_sac_vec,
  sistema = "SAC"
)

df_plot <- dplyr::bind_rows(df_price, df_sac)

p <- ggplot(df_plot, aes(x = mes, y = parcela, color = sistema, group = sistema)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Evolução das Parcelas nos Primeiros 24 Meses",
    subtitle = paste0("Financiamento de R$ ", classtools::format_cash(principal), ", 60 meses, CET ", classtools::format_percent(r_annual), " a.a."),
    x = "Mês",
    y = "Valor da Parcela (R$)",
    color = "Sistema"
  ) +
  scale_y_continuous(labels = classtools::format_cash) +
  scale_x_continuous(breaks = seq(1, n_plot, by = 2)) +
  theme_light() +
  theme(legend.position = "bottom")

p
```

## Amortização Extraordinária no SAC

```{r}
extra_month <- 12
extra_amount <- 15000

pmt_normal <- sapply(1:n_months, function(m) amort_sac + (principal - (m - 1) * amort_sac) * r_monthly)

sd_m12_new <- (principal - 12 * amort_sac) - extra_amount
amort_sac_new <- sd_m12_new / (n_months - extra_month)

pmt_extra <- c(
  sapply(1:extra_month, function(m) amort_sac + (principal - (m - 1) * amort_sac) * r_monthly),
  sapply(1:(n_months - extra_month), function(m) amort_sac_new + (sd_m12_new - (m - 1) * amort_sac_new) * r_monthly)
)

total_normal <- sum(pmt_normal)
total_extra <- sum(pmt_extra)
economia <- total_normal - total_extra - extra_amount

df_comp <- tibble::tibble(
  mes = rep(1:n_months, 2),
  parcela = c(pmt_normal, pmt_extra),
  cenario = c(rep("Sem Amortização Extra", n_months), rep("Com Amortização de R$ 15k no mês 12", n_months))
)

p_extra <- ggplot(df_comp, aes(x = mes, y = parcela, color = cenario)) +
  geom_line(linewidth = 1.2) +
  geom_vline(xintercept = 12, linetype = "dashed", color = "red") +
  labs(
    title = "Efeito da Amortização Extraordinária de R$ 15.000 (Mês 12)",
    subtitle = paste0("Economia líquida de juros: ", classtools::format_cash(economia)),
    x = "Mês",
    y = "Valor da Parcela (R$)",
    color = "Cenário"
  ) +
  scale_y_continuous(labels = classtools::format_cash) +
  theme_light() +
  theme(legend.position = "bottom")

p_extra
```

## Cuidados em um financiamento imobiliário

> Financiamento imobiliário é um empréstimo aceitável para pessoa física, desde que seja feito com alguns cuidados. Porém, o sonho do imóvel próprio pode se tornar um pesadelo.

::: {.incremental}
- **Escolha um imóvel que caiba no seu orçamento!!**

- Nunca financie mais que 75% do imóvel, **sempre dê a maior entrada possível**
  - quanto maior a entrada, menor as parcelas e menor a chance de devolver o imóvel
  - quanto menor a entrada, maior o risco de inadimplência e **perda do imóvel**

- Nunca financie mais que 30% da sua renda, mesmo que o banco permita. Sua renda hoje não é garantida para sempre (exceto alguns casos especiais)

- Dê preferência a tabela SAC, pois as parcelas são decrescentes e o valor total pago é menor

- Sempre que possível, amortize o financiamento com recursos extras, pois o valor total pago será menor

- Desde o início, tenha um plano B caso perca o emprego ou tenha uma queda de renda.

:::

## Referências