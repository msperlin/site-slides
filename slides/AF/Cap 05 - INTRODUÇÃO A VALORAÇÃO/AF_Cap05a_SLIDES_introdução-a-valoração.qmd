...
format:
  revealjs: 
    include-after-body: "themes/clock.txt"
...
include-in-header:
  - text: |
     <link rel = "shortcut icon" href = "templates/ufrgs3.jpg" />  
---
...
df_td <- GetTDData::td_get("NTN-B Principal", dl_folder = dl_folder)
...
## O caso Barsi (o efeito dos juros compostos)

```{r}
#| echo: false


dob_barsi <- as.Date("1939-03-10")
age_barsi <- lubridate::year(Sys.Date()) - lubridate::year(dob_barsi) 

idade_inicial <- 30
patr_estimado <- 6000000000
anos <- age_barsi - idade_inicial
meses <- anos*12
aporte <- 3000
inicial <- 100000
r <- 0.1875
...
> Luiz Barsi, o **Rei dos Dividendos**, é um dos maiores investidores individuais da bolsa de valores brasileira.

- Última estimativa de patrimônio em torno de `r classtools::format_cash(patr_estimado)`

- Começou a investir com `r idade_inicial` anos e hoje tem `r age_barsi` (são `r anos` anos investindo).

- Pela matemática financeira, o  patrimonio dele pode ser atingido se ele iniciar os investimentos com `r classtools::format_cash(inicial)` e investir `r  classtools::format_cash(aporte)` reais mensalmente, a uma taxa de juros real e constante de `r classtools::format_percent(r)`. Ao ano, o final dos anos teria `r classtools::format_cash(P[length(P)])`
