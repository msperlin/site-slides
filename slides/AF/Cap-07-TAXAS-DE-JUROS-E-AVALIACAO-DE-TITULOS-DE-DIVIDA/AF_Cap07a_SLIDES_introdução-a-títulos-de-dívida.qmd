---
title: "Introdução a Títulos de Dívida"
...
df_curr <- df_curr |>
  dplyr::filter(!stringr::str_detect(asset_code, "Renda+|Educa+|IGPM+")) |>
  dplyr::arrange(asset_code)

tbl <- df_curr |>
  dplyr::rename(Maturidade = matur_date,
                Nome = asset_code,
                `Valor Título` = price_bid,
                `Taxa Anual` = yield_bid) |>
  dplyr::select(Nome, Maturidade,
