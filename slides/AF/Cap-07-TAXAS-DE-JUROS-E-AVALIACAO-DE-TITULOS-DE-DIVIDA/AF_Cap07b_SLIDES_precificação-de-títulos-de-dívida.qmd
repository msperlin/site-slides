---
title: "Precificação de Títulos de Dívida"
...
format:
  revealjs: 
    include-after-body: "resources/clock.txt"
...
Todos os cálculos disponíveis no [GSheet](https://docs.google.com/spreadsheets/d/12pu98nVWtEQJaEL03m-42M5HfTPTAPn6L-cEsuZU9qc/edit?usp=sharing).


## Representação de um Título de Renda Fixa com cupons

```{r}
#| fig-cap: "Exemplo de título de dívida com cupom [(fonte)](https://www.tesourodireto.com.br/titulos/tipos-de-tesouro.htm)"
...
df_td <- GetTDData::td_get_current() |>
  dplyr::filter(
    stringr::str_detect(asset_code, "LTN 010127"))

df_td |> 
  dplyr::select(ref_date,asset_code, price_bid, yield_bid) |>
  gt::gt() |>
  gt::cols_label(
    .list = list(
      asset_code = "Código do Título",
      ref_date = "Data de Referência",
      price_bid = "Preço de Compra",
      yield_bid = "Yield de Compra")
  ) |>
  gt::fmt_percent("yield_bid") |>
  gt::tab_header(
...
# data de referência da última pesquisa mensal do BCB
data_ref <- as.Date("2026-07-01")
url <- paste0(
  "https://www.bcb.gov.br/api/servico/sitebcb/historicotaxajurosdiario/TodosCampos",
  "?filtro=codigoSegmento%20eq%20%271%27%20and%20codigoModalidade%20eq%20%27401101%27",
  "%20and%20InicioPeriodo%20eq%20%27", data_ref, "%27"
)
...
tbl |>
  ...
  gt::gt() |>
  gt::tab_header(
    title = gt::md("10 **Maiores** Taxas de Financiamento | Automóveis (Pessoa Física)"),
    subtitle = glue::glue("Dados de referência: {classtools::format_date(data_ref)}")
  ) |>
...
tbl |>
  ...
  gt::gt() |>
  gt::tab_header(
    title = gt::md("10 **Menores** Taxas de Financiamento | Automóveis (Pessoa Física)"),
    subtitle = glue::glue("Dados de referência: {classtools::format_date(data_ref)}")
  ) |>
  gt::fmt_percent(Juros_AA)
```
...
Fonte: [Banco Central](https://www.bcb.gov.br/estatisticas/reporttxjuros?codigoSegmento=1&codigoModalidade=401101) (`r classtools::format_date(data_ref)`)
...
df_taxas_imovel <- df_raw |>
  ...
  filter(Juros_AA != 0) |>
  # remove a taxa de 'mercado' da CAIXA (mantém a taxa regulada, exibida abaixo)
  filter(!(MODALIDADE == "FINANCIAMENTO IMOBILIÁRIO COM TAXAS DE MERCADO - PREFIXADO" &
...
p <- ggplot(df_both, aes(x = year, y = r_aa, color = series)) + 
  ...
  theme_light() + 
  labs(title = glue(
    "IPCA e SELIC no Brasil ({first_date} -> {last_date})"
    ),
    y = "Taxa anual (IPCA acumulado e SELIC)",
       x = "Anos",
       caption = "Dados do sistema SGS-BCB") 
...
# guarda os dados baixados do B3 em cache local (evita novo download a cada compilação).
# precisa ser definido ANTES de carregar o pacote rb3.
...
# datas de referência: primeiro dia útil de cada ano (últimos 20 anos) +
# primeiro dia útil de cada mês do ano corrente
ref_dates <- unique(c(
  getdate("first bizday", 2006:2025, "Brazil/ANBIMA"),
  getdate("first bizday",
          seq(as.Date(format(Sys.Date(), "%Y-01-01")), Sys.Date(), by = "month"),
          "Brazil/ANBIMA")
))

# baixa as curvas de juros nominais (PRE) direto do B3
fetch_marketdata("b3-reference-rates", refdate = ref_dates)
