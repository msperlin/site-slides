...
> O operador de _pipeline_ é usado com os símbolos `|>` (nativo com R > 4.1) e `%>%` (pacote `magrittr`) e permite que dataframes sejam criados em etapas consecutivas
...
4. Utilize o pacote `yfR` para baixar dados da ação da META/Facebook (META) desde a data `'2010-01-01'` até hoje. Filtre os dados para manter apenas dados onde o dia da semana era segunda-feira.
...
6. Crie um objeto do tipo lista com três dataframes em seu conteúdo, `df1`, `df2` e `df3`. O conteúdo e tamanho dos dataframes é livre. Utilize a função `sapply` para descobrir o número de linhas e colunas em cada dataframe.


## Tarefa 03 - Integração de Mercado

7. Usando o pacote `yfR`, baixe os dados da Petrobras (PETR4.SA) e do índice Ibovespa (^BVSP) em dois dataframes diferentes (duas chamadas da função `yfR::yf_get`). Empilhe os dados das duas ações com o comando `dplyr::bind_rows`.
