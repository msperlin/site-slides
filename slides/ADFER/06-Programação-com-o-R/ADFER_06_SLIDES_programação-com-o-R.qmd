...
- permite iterações sequenciais, encasulamento de métodos, e automatização de qualquer procedimento computacional
...
> Vamos criar uma função que calcule, para qualquer ação do YF, o retorno acumulado entre 2015 e a data atual
...
> Utilizados de forma ostensiva na análise de dados, os _loops_ permitindo a repetição estruturada de códigos e possibilitando o processamento de itens individualmente de uma forma fácil.
...
## Coletando retorno acumulados com loops
...
- Outro exemplo prático do uso de um _loop_ é o processamento de dados de acordo com grupos. Por exemplo, para dados de preços de diferentes ações da Bovespa, caso quiséssemos calcular o preço médio de cada ação, construiríamos o seguinte código para importar os dados e processá-los:
...
- No `tidyverse`, programação funcional é oferecida pelo pacote `r classtools::cite_pkg("purrr")`
- O foco aqui será com as funções do pacote `r classtools::cite_pkg("purrr", clean_up = TRUE)`
:::


## Pacote `r classtools::cite_pkg("purrr")`

Principais funções:

- `map()`: retorna sempre uma lista
- `map_dbl()`: retorna sempre  um vetor numérico (_double_)
- `map_chr()`: retorna sempre  um vetor de text (_character_)
...
Agora vamos utilizar `safely` para encapsular `my_fct` em outra função chamada `my_fct_safely`:
...
## Operações de Grupo com `r classtools::cite_pkg("dplyr")`
...
05. Use o pacote `yfR` para baixar dados do índice SP500 (`'^GSPC'`), Ibovespa (`'^BVSP'`), FTSE (`'^FSTE'`) e Nikkei 225 (`'^N225'`) de `'2010-01-01'` até a data atual. Com os dados importados, use um _loop_ para calcular o retorno médio, máximo e mínimo de cada índice durante o período analisado. Salve todos os resultados em uma tabela única e a mostre no _prompt_ do R.

06. Refaça o exercício anterior utilizando as funções `group_by` e `summarise`, ambas do pacote `dplyr`.

07. No site do [Rstudio CRAN logs](http://cran-logs.rstudio.com/) você encontrará  dados sobre as estatísticas de download para a distribuição de base de R na seção _Daily R downloads_. Usando suas habilidades de programação, importe todos os dados disponíveis entre `r "2015-01-01"` e `r Sys.Date()` e agregue-os em uma única tabela. Qual país apresenta a maior contagem de downloads do R? NOTA: caso tiver erro no download, ignore os arquivos com falha, e continue a importaçẽo e análise dos demais. Podes usar `purrr::safely` para tratar erros no código.
