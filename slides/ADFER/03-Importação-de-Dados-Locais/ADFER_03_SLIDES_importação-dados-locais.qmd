...
- **Pacotes Recomendados:**
  - `arrow` ([Apache Arrow](https://arrow.apache.org/docs/r/)): manipulação de arquivos colunares (`.parquet`) e datasets particionados em disco, com suporte nativo à sintaxe do `dplyr` sem carregar tudo na RAM.
  - `duckdb`: motor SQL analítico (*OLAP*) embutido e de altíssimo desempenho, consultando arquivos locais e integrando perfeitamente com `dplyr` / `dbplyr`.
  - `data.table`: leitura ultrarrápida de arquivos texto com `fread()` e manipulação altamente otimizada com uso eficiente de memória.
  - `fst`: leitura e gravação paralela veloz para data frames locais.
...
2. Experimente modificar o código anterior alterando:
   ...
   - A quantidade e tipos de colunas (adicione variáveis de texto como `name` e `sentence`, ou mais variáveis numéricas/datas com `wakefield`).
